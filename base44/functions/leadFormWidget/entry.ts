...
Deno.serve(async (req) => {
  ...
  js += "    }\n";
  js += "    var services = Array.isArray(config.lead_form_services) ? config.lead_form_services : [];\n";
  js += "    if (config.lead_form_show_service && services.length) {\n";
  js += "      h += '<div class=\"alf-field\"><label class=\"alf-label\" for=\"alf-service\">Storitev</label>';\n";
  js += "      h += '<select class=\"alf-input\" id=\"alf-service\"><option value=\"\">Izberite storitev…</option>';\n";
  js += "      for (var i = 0; i < services.length; i++) { h += '<option value=\"' + escHtml(services[i]) + '\">' + escHtml(services[i]) + '</option>'; }\n";
  js += "      h += '</select></div>';\n";
  js += "    }\n";
  js += "    if (config.lead_form_show_message !== false) {\n";
  js += "      h += '<div class=\"alf-field\"><label class=\"alf-label\" for=\"alf-message\">Sporočilo <span style=\"color:#ef4444\">*</span></label>';\n";
  js += "      h += '<textarea class=\"alf-input\" id=\"alf-message\" rows=\"3\" placeholder=\"Vaše povpraševanje…\"></textarea></div>';\n";
  js += "    }\n";
  ...
  js += "      var serviceEl = document.getElementById('alf-service');\n";
  ...
  js += "      if (msgEl && !msgEl.value.trim()) { errEl.textContent = 'Sporočilo je obvezno.'; errEl.style.display = 'block'; return; }\n";
  js += "      if (consentEl && !consentEl.checked) { errEl.textContent = 'Za pošiljanje potrdite soglasje.'; errEl.style.display = 'block'; return; }\n";
  ...
  js += "      var msgParts = [];\n";
  js += "      if (serviceEl && serviceEl.value) msgParts.push('Storitev: ' + serviceEl.value);\n";
  js += "      if (msgEl && msgEl.value.trim()) msgParts.push(msgEl.value.trim());\n";
  js += "      if (msgParts.length) payload.message = msgParts.join('\\n');\n";
